...
def driver(data_config_file, param_config_file, random_seed_index,  zero_init = False, oppo_asymmetry = False, set_start = False, pop_override = False):
    ...
    for layer in range(data_layer + 1, max_mer):
        ...
        if layer % 2 == 1:
            # if the layer is odd (meaning the mer size is even), just need to populate the matrix and move on
            # the two relevant matrices are the set_p_vec_sample_matrix and old_set_probabilities
            continue
...
def init_set_edges(set_probability_vector, current_set_p_vec_array, current_p_vec_array, set_p_vec_sample_matrix, leaf_likelihood_array, old_set_probabilities_matrix, phi_array, leaf_counts_array, layer_size, zero_contexts, suppress): 
    ...
    for parent_i in range(total_parent_contexts):
        ...
        new_parent_p_vec_array = np.zeros((16, 3))
        new_children_p_vecs = np.zeros((16, 3))
        for i in range(16):
            new_parent_p_vec_array[i] = new_parent_p_vec
            new_children_p_vecs[i] = new_parent_p_vec * np.exp(phi_array[parent_i * 16 + i])
        ...
        current_set_p_vec_array[parent_i * 16: parent_i * 16 + 16] = new_parent_p_vec_array
        current_p_vec_array[parent_i * 16: parent_i * 16 + 16] = new_children_p_vecs
        ...
        for i in range(16):
            set_probability_vector[parent_i * 16 + i] = old_set_probabilities_matrix[random_sample_index, parent_i, :] * 0.25
...
def project_set_p_vec_sample_array(parent_layer_set_p_vec_sample_array, layer_size):
    ...
    for parent_set_p_vec_sample in parent_layer_set_p_vec_sample_array:
        for index_mod in range(16):
             array_index = index_mod + (parent_index * 16)
...
def initialize_layer_tree_edge_list(layer, layer_size, max_mer, old_context_list, sigmas, alpha_probs, set_p_vec_sample_array, leaf_count_dict, first_sample, random_seed, zero_init, oppo_asymmetry = False):
    ...
    if layer == 0:
        ...
        for parent_context in old_context_list:
            ...
            for nuc_combo in ['AA', 'AC', 'AG', 'AT', 'CA', 'CC', 'CG', 'CT','GA', 'GC', 'GG', 'GT','TA', 'TC', 'TG', 'TT',]:
                array_index = index_mod + (parent_index * 16)
                
                
                child_context = nuc_combo[0] + parent_context + nuc_combo[1]
